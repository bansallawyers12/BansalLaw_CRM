...
document.addEventListener('DOMContentLoaded', function() {
     ...
     * Shows a matter-reopen-style modal (not iziToast) with:
     *   - View event / Dismiss per item
     *   - Remind me (snooze 5–60 min) — alert returns when the snooze ends
     * Session dismiss + localStorage snooze keep alerts from looping.
     * ─────────────────────────────────────────────────────────────────── */
    ...
    function bookingCalReminderOpenEvent(eventId) {
        ...
        if (typeof iziToast !== 'undefined') {
            iziToast.info({
                title: 'Event',
                message: 'Could not open that event on the calendar. Try clicking it directly.',
                position: 'topRight',
                timeout: 4000,
            });
        }
    ...
    function bookingCalReminderBindModalOnce() {
        ...
        modalEl.addEventListener('hidden.bs.modal', function () {
            ...
            // Closing without Remind me / View: soft-hide for 2 min so it does not loop every poll
    ...
    async function bookingCalPollReminders() {
        try {
            ...
            if (isOpen) {
                // Merge any newly due items into the open modal
                const seen = {};
