@php
    $mode = $mode ?? 'create';
    $isEdit = $mode === 'edit';
    $fieldPrefix = $fieldPrefix ?? ($isEdit ? 'edit_mdt' : 'create_mdt');
    $fetchedData = $fetchedData ?? null;
    $accordionId = $fieldPrefix . '_accordion';
@endphp

<div class="mdt-modal-accordion" id="{{ $accordionId }}">
    <div class="accordion">
        ...
        <div class="accordion-body collapse show" id="{{ $fieldPrefix }}_primary" data-bs-parent="#{{ $accordionId }}">
            <div class="row g-3">
                ...
                <div class="col-12">
                    <p class="text-muted small mb-0">Folders without a specific client or matter are available broadly across matters.</p>
                </div>
            </div>
        </div>
    </div>
