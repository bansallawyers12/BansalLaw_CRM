(function ($) {
    ...
    var $listContent = $('#mdt-list-content');
    var $listFooter = $('#mdt-list-footer');
    var $listLoading = $('#mdt-list-loading');
    var $searchInput = $('#mdt-search-input');
    ...
    function updateBrowserUrl() {
        ...
        window.history.replaceState({ mdtSearch: state.search }, '', urls.index + (query ? ('?' + query) : ''));
    }
    ...
    function loadList(page) {
        ...
        $.ajax({
            ...
            if (!response || !response.success) {
                showFlashMessage((response && response.message) || 'Failed to load folders.', 'error');
                return;
            }
            ...
            showFlashMessage((xhr.responseJSON && xhr.responseJSON.message) || 'Failed to load folders.', 'error');
        }).always(function () {
            setLoading(false);
        });
    }
    ...
    function openCreateModal() {
        var $modal = $('#mdtCreateModal');
        var $body = $('#mdt-create-form-body');
        var $alert = $('#mdt-create-alert');
        ...
        clearFormErrors($('#mdt-create-form'), $alert);
        showModal($modal);
    ...
    function openEditModal(itemId) {
        var $modal = $('#mdtEditModal');
        var $body = $('#mdt-edit-form-body');
        var $alert = $('#mdt-edit-alert');

        state.editItemId = itemId;
        $('#mdt_edit_id').val(itemId);
        $body.html('<div class="text-center py-5 text-muted"><div class="spinner-border spinner-border-sm me-2" role="status"></div>Loading form...</div>');
        clearFormErrors($('#mdt-edit-form'), $alert);
        showModal($modal);
        ...
        $.ajax({
            ...
            $('#mdtEditModalLabel').text('Edit ' + (response.item && response.item.title ? response.item.title : 'folder'));
            $body.html(response.html || '');
        }).fail(function (xhr) {
    ...
    function openViewModal(itemId) {
        var $modal = $('#mdtViewModal');
        var $body = $('#mdt-view-body');
        ...
        $.ajax({
            ...
            if (!response || !response.success) {
                $body.html('<div class="alert alert-danger mb-0">' + escapeHtml((response && response.message) || 'Could not load folder.') + '</div>');
                return;
            }
            $('#mdtViewModalLabel').text((response.item && response.item.title) || 'Folder details');
            $body.html(response.html || '');
        }).fail(function (xhr) {
            $body.html('<div class="alert alert-danger mb-0">' + escapeHtml((xhr.responseJSON && xhr.responseJSON.message) || 'Could not load folder.') + '</div>');
        });
    }
    ...
    $('#mdt-search-btn').on('click', function () {
        state.search = $.trim($searchInput.val());
        state.page = 1;
        loadList(1);
    });

    $('#mdt-search-clear').on('click', function () {
        $searchInput.val('');
        state.search = '';
        state.page = 1;
        loadList(1);
    ...
    $searchInput.on('keydown', function (event) {
        if (event.key === 'Enter') {
            ...
            $('#mdt-search-btn').trigger('click');
        }
    });

    $(document).on('click', '#mdt-list-pagination a', function (event) {
    ...
    $('#mdt-add-btn').on('click', openCreateModal);

    $(document).on('click', '.mdt-edit-btn', function () {
        openEditModal($(this).data('mdt-id'));
    });

    $(document).on('click', '.mdt-view-btn', function () {
        openViewModal($(this).data('mdt-id'));
    });

    $('#mdt-edit-from-view').on('click', function () {
        var itemId = state.currentItemId;
        if (!itemId) {
            return;
        }
        hideModal($('#mdtViewModal'));
        setTimeout(function () {
            openEditModal(itemId);
        }, 250);
    });

    $('#mdt-view-from-edit').on('click', function () {
        var itemId = state.editItemId;
        if (!itemId) {
            return;
        }
        hideModal($('#mdtEditModal'));
        setTimeout(function () {
            openViewModal(itemId);
        }, 250);
    });

    $('#mdt-create-form').on('submit', function (event) {
        event.preventDefault();
        submitForm($(this), urls.store, $('#mdt-create-alert'), $('#mdt-create-submit'), function (response) {
            hideModal($('#mdtCreateModal'));
            showFlashMessage(response.message, 'success');
            loadList(1);
        });
    });

    $('#mdt-edit-form').on('submit', function (event) {
        event.preventDefault();
        var itemId = state.editItemId || $('#mdt_edit_id').val();
        if (!itemId) {
            return;
        }
        submitForm(
