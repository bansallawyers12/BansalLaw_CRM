...
{
    use HasFactory, Notifiable, SortableTrait, HasApiTokens;
    ...
    /**
     * Role IDs that may grant {@see canAssignEmailsBySubject()} to others.
     * Default: native Super Admin (1) only.
     */
    public static function assignEmailsBySubjectGrantRoleIds(): array
    {
        $ids = config('crm.assign_emails_by_subject_grant_role_ids', [1]);

        return is_array($ids) ? array_values(array_map('intval', $ids)) : [1];
    }

    /**
     * Whether the actor may toggle Assign by subject on staff records.
     * Only native Super Admin (role 1) may grant this.
     */
    public static function canGrantAssignEmailsBySubjectPermission(?self $actor): bool
    {
        if (! $actor instanceof self) {
            return false;
        }

        return in_array((int) ($actor->role ?? 0), self::assignEmailsBySubjectGrantRoleIds(), true);
    }

    /**
     * Native Super Admin, or staff with the per-user grant, may use Assign by
     * subject on Unassigned Mail (preview + confirm assignment).
     */
    public function canAssignEmailsBySubject(): bool
    {
        if ((int) ($this->role ?? 0) === 1) {
            return true;
        }

        return (bool) ($this->can_assign_emails_by_subject ?? false);
    }

    /**
     * Role IDs that may grant {@see canUseCommunicationCheck()} to others.
     * Default: native Super Admin (1) only.
     */
    public static function communicationCheckGrantRoleIds(): array
    {
        $ids = config('crm.communication_check_grant_role_ids', [1]);

        return is_array($ids) ? array_values(array_map('intval', $ids)) : [1];
    }

    /**
     * Whether the actor may toggle Communication Check on staff records.
     * Only native Super Admin (role 1) may grant this.
     */
    public static function canGrantCommunicationCheckPermission(?self $actor): bool
    {
        if (! $actor instanceof self) {
            return false;
        }

        return in_array((int) ($actor->role ?? 0), self::communicationCheckGrantRoleIds(), true);
    }

    /**
     * Native Super Admin, or staff with the per-user grant, may use Communication Check
     * (screenshot extract → CRM match → Logged/Worked/Gap). Feature flag / env gate still apply.
     */
    public function canUseCommunicationCheck(): bool
    {
        if ((int) ($this->role ?? 0) === 1) {
            return true;
        }

        return (bool) ($this->can_use_communication_check ?? false);
    }

    /**
     * Role IDs that may grant {@see canUseTimelineBilling()} to others.
     * Default: native Super Admin (1) only.
     */
    public static function timelineBillingGrantRoleIds(): array
    {
        $ids = config('crm.timeline_billing_grant_role_ids', [1]);

        return is_array($ids) ? array_values(array_map('intval', $ids)) : [1];
    }

    /**
     * Whether the actor may toggle Timeline Billing on staff records.
     * Only native Super Admin (role 1) may grant this.
     */
    public static function canGrantTimelineBillingPermission(?self $actor): bool
    {
        if (! $actor instanceof self) {
            return false;
        }

        return in_array((int) ($actor->role ?? 0), self::timelineBillingGrantRoleIds(), true);
    }

    /**
     * Super Admin (including elevated), or staff with the per-user grant, may use
     * Timeline Billing on the client activity feed.
     */
    public function canUseTimelineBilling(): bool
    {
        if ($this->hasEffectiveSuperAdminPrivileges()) {
            return true;
        }
