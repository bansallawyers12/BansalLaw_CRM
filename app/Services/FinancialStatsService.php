...
class FinancialStatsService
{
    ...
    public function getMonthlyStats($startDate, $endDate, $receiptType = null)
    {
        ...
        $totalDeposits = $receiptType === null || $receiptType == 1
            ? $applyDateFilter($applyTypeFilter($this->applyActiveClientFundsLedgerFilter(DB::table('account_client_receipts')->where('receipt_type', 1))), $startDateStr, $endDateStr)
                ->sum('deposit_amount')
            : 0;

        // Total fee transfers this month (only for Client Receipts)
        $totalFeeTransfers = $receiptType === null || $receiptType == 1
            ? $applyDateFilter($this->applyActiveClientFundsLedgerFilter(DB::table('account_client_receipts')
    ...
    public function getReceiptStats($startDate, $endDate, $receiptType = null)
    {
        ...
        $unallocatedCount = $this->applyActiveClientFundsLedgerFilter(DB::table('account_client_receipts')
            ->where('receipt_type', 1))
        ...
        $allocatedCount = $this->applyActiveClientFundsLedgerFilter(DB::table('account_client_receipts')
            ->where('receipt_type', 1))
            ->whereNotNull('trans_date')
            ->whereRaw("TO_DATE(trans_date, 'DD/MM/YYYY') BETWEEN TO_DATE(?, 'DD/MM/YYYY') AND TO_DATE(?, 'DD/MM/YYYY')", [$startDateStr, $endDateStr])
        ...
        $totalClientReceipts = $this->applyActiveClientFundsLedgerFilter(DB::table('account_client_receipts')
            ->where('receipt_type', 1))
            ->whereNotNull('trans_date')
            ->whereRaw("TO_DATE(trans_date, 'DD/MM/YYYY') BETWEEN TO_DATE(?, 'DD/MM/YYYY') AND TO_DATE(?, 'DD/MM/YYYY')", [$startDateStr, $endDateStr])
            ->count();
    ...
    public function getAllocationMetrics($startDate, $endDate, $receiptType = null)
    {
        ...
        $validatedReceipts = $this->applyActiveClientFundsLedgerFilter(DB::table('account_client_receipts')
            ->where('receipt_type', 1))
            ->whereNotNull('trans_date')
            ->whereRaw("TO_DATE(trans_date, 'DD/MM/YYYY') BETWEEN TO_DATE(?, 'DD/MM/YYYY') AND TO_DATE(?, 'DD/MM/YYYY')", [$startDateStr, $endDateStr])
            ->where('validate_receipt', 1)
        ...
        $oldUnallocated = $this->applyActiveClientFundsLedgerFilter(DB::table('account_client_receipts')
            ->where('receipt_type', 1))
            ->whereNotNull('trans_date')
            ->whereRaw("TO_DATE(trans_date, 'DD/MM/YYYY') BETWEEN TO_DATE(?, 'DD/MM/YYYY') AND TO_DATE(?, 'DD/MM/YYYY')", [$startDateStr, $endDateStr])
            ->where(function($q) {
    ...
    public function getTrendData($receiptType = null)
    {
        ...
        for ($i = 5; $i >= 0; $i--) {
            ...
            $deposits[] = ($receiptType === null || $receiptType == 1)
                ? $this->applyActiveClientFundsLedgerFilter(DB::table('account_client_receipts')
                    ->where('receipt_type', 1))
                    ->whereNotNull('trans_date')
                    ->whereRaw("TO_DATE(trans_date, 'DD/MM/YYYY') BETWEEN TO_DATE(?, 'DD/MM/YYYY') AND TO_DATE(?, 'DD/MM/YYYY')", [$startDateStr, $endDateStr])
                    ->sum('deposit_amount')
    ...
    /**
     * Client funds ledger rows that still affect balances (not voided fee transfer).
     */
    private function applyActiveClientFundsLedgerFilter($query)
    {
